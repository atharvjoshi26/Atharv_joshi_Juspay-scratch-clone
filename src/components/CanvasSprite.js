...
import { checkAndSwapCollision, goTo, move, resetCollisionHandled, rotate,resetSpritePositions, moveInX, moveInY, rotateClockwise, rotateCounterClockwise } from "../redux/spritesSlice";
import { PlayCircle } from "lucide-react";
import { GO_TO, MOVE_STEPS, REPEAT, TURN_DEGREES,MOVE_IN_X,MOVE_IN_Y,TURN_CLOCKWISE,TURN_COUNTERCLOCKWISE } from "../constants/sidebarBlocks";

export default function CanvasSprite() {
    ...
    const executeAction = ({ spriteId, type, payload }) => {
        const actionMap = {
            ...
            [MOVE_IN_X]: () => dispatch(moveInX({ spriteId, ...payload })),    
            [MOVE_IN_Y]: () => dispatch(moveInY({ spriteId, ...payload })),
            [TURN_DEGREES]: () => dispatch(rotate({ spriteId, ...payload })),
            [TURN_CLOCKWISE]: () => dispatch(rotateClockwise({ spriteId, degree: payload.degree })),
            [TURN_COUNTERCLOCKWISE]: () => dispatch(rotateCounterClockwise({ spriteId, degree: payload.degree })),
            ...
            if (sprites.length > 1 && spritesState.showCollisionAnimation && !spritesState.collisionHandled) {
                sprites.forEach((sprite2) => {
                    if (sprite2.name !== sprite.name) {
                        dispatch(checkAndSwapCollision({ spriteId1: sprite.id, spriteId2: sprite2.id }));
                    }
                });
            }
    ...
    const handleDragAction = useCallback((spriteId) => {
