...
import { chooseSprite } from '../redux/spritesSlice';
import { SPRITE_HEIGHT, SPRITE_WIDTH } from '../constants/sprites';

export const SpriteIcon = ({ spriteName, styles, handleClick }) => {
    switch (spriteName) {
        case "cat":
...
const Sprite = ({ sprite, containerSize, onDragStart, onDrag }) => {
    ...
    const handleClick = (e) => {
        e.preventDefault();
        dispatch(chooseSprite(sprite.id));
    };
    ...
    return (
        <div
            ...
            onDrag={onDrag}
        >
            <SpriteIcon spriteName={sprite.name} handleClick={handleClick} styles={{ width: SPRITE_WIDTH + "px", height: SPRITE_HEIGHT + "px" }} />
        </div>
    );
};
